...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return (
    <span
      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${map[priority] ?? "bg-gray-100 text-gray-500"}`}
    >
      {priority}
    </span>
  );
...
export default function AdminDashboard() {
  ...
  const isLoading =
    dashLoading || projectsLoading || tasksLoading || membersLoading;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        />
        <StatCard
          icon={<ClipboardList size={22} className="text-yellow-500" />}
          label="Active Tasks"
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.slice(0, 6).map((project) => {
            ...
            const inProg = project.tasks.filter(
            ...
            <thead>
              <tr className="border-b border-gray-100">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left text-xs text-gray-400 font-medium pb-3 pr-4 last:pr-0"
                  >
                    {h}
                  </th>
                ))}
              ...
              {recentTasks.map((task) => (
                ...
                >
                  ...
                  <td className="py-3">
                    {task.dueDate ? (
                      <span
                        className={`text-xs ${new Date(task.dueDate) < new Date() && task.status !== "COMPLETED" ? "text-red-500 font-semibold" : "text-gray-500"}`}
                      >
